...
using Circumference.ImportAndExport.Excel;
...
namespace Web.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async  Task<ActionResult> Upload(HttpPostedFileBase file)
        {
            ...
            filePath = Server.MapPath(string.Format("~/{0}/{1}", "File",fileName));
            var import =await Importer.Import<ImportBillsDto>(filePath);
            return Json("上传成功");
        }
        ...
        {
            //fileName = "OverAllBills";
            //FileStream fs=default(FileStream);
            //string filePath = Server.MapPath(string.Format("~/{0}/{1}", "FileTemplate", fileName));
            ...
            //else
            //{
            //      var rrr= await Importer.GenerateTemplateByte<ImportBillsDto>();
            //}

            //return File(fs, "text/plain", fileName);
            var fs = await Importer.GenerateTemplateByte<ImportBillsDto>();
